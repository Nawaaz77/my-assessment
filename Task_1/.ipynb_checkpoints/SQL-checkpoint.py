...
def question_1():
    ...
    qry = """Select Name, Surname, CustomerID  FROM customers GROUP BY CustomerID, Name, Surname Having COUNT(CustomerID) > 1"""
...
def question_3():
    ...
    qry = """Select LoanTerm, (Count(CASE WHEN ApprovalStatus = 'Approved' THEN 1 END)/Count(LoanTerm)) * 100.0 AS PercentageApproved FROM loans GROUP BY LoanTerm"""
...
def question_4():
    ...
    qry = """Select CustomerClass, Count(DISTINCT(CustomerID)) AS Count FROM credit GROUP BY CustomerClass ORDER BY CustomerClass"""
